...
class CifarFrame(Entity):
    """
    Frame to display text or numbers on a wall
    Note: the position is in the middle of the frame, on the wall
    """

    def __init__(self, pos, dir, str, height=2.5, depth=0.05):
        super().__init__()

        self.pos = pos
        self.dir = dir

        self.classes = ['cat','deer','dog']
    ...
    def randomize(self, params, rng):
        ...
        try:
            cifarclass = np.random.randint(3)
            pic_id = str(np.random.randint(1000)).zfill(4)
            tex_name = '{}/{}'.format(self.classes[cifarclass],pic_id)
            self.texs.append(Texture.get(tex_name, rng,cifar=True))
        except:
...
class TextFrame(Entity):
    ...
    def __init__(self, pos, dir, str, height=1.15, depth=0.05):
        super().__init__()
